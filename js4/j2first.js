...
document.getElementById('gotofa').addEventListener("click",function(e){
    ...
    if(!reg.test(document.getElementById('renshu').value)){
        ...
        var shenfendata2 = JSON.parse(localStorage.getItem('shashou1'));
        var pingmin = 0;
       
        var shashou = 0;
        for(let i=0;i<shenfendata2.length;i++){
            if(shenfendata2[i]=='杀手'){
                shashou++;
            }
            else{
                pingmin++;
            }
            localStorage.shashou = shashou;
            localStorage.pingmin = pingmin;
        }
